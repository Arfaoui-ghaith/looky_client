...
export default function AppointmentsTable({ isLoading, appointments }) {
  ...
  const [
    visibleFeedbackUnavailableDialog,
    setVisibleFeedbackUnavailableDialog,
  ] = useState(false);
  ...
  const footerConfirmContent = (
    <div>
      <Btn
        label="No"
        icon="pi pi-times"
        onClick={() => setVisibleFeedbackDialog(false)}
  ...
  const footerFeedbackUnavailableContent = (
    <div>
      <Btn
        label="Ok"
        onClick={() => setVisibleFeedbackUnavailableDialog(false)}
        className="p-button-text"
      />
    </div>
  );
    ...
    if (hasSearchFilter) {
      filteredAppointments = filteredAppointments.filter(
        (appointment) =>
          ...
          appointment.email.toLowerCase().includes(filterValue.toLowerCase()),
      );
    }
    if (
      statusFilter !== "all" &&
        ...
        return (
          <div className="flex flex-col">
            <p className="text-bold text-small capitalize mb-0 mt-auto pt-2">
              {cellValue.toLocaleDateString("fr-FR")}
            </p>
    ...
    <div className="m-5">
      ...
      <div className="card flex justify-content-center">
        <Dialog
          ...
          style={{ width: "30vw" }}
          onHide={() => setVisibleFeedbackDialog(false)}
          footer={footerConfirmContent}
        >
          <p className="m-2" style={{ color: "#fff" }}>
            <strong className="mx-auto" style={{ color: "#EB1616" }}>
              Barber Shop
            </strong>{" "}
          ...
          <p className="m-2" style={{ color: "#fff" }}>
            ...
            : {new Date(selectedAppointment?.date).getUTCHours()}h
            {new Date(selectedAppointment?.date).getUTCMinutes()}
          </p>
          <p className="m-2" style={{ color: "#fff" }}>
            <strong className="mx-auto" style={{ color: "#EB1616" }}>
              Status
      ...
      <div className="card flex justify-content-center">
        <Dialog
          header="Feedback Unavailable"
          visible={visibleFeedbackUnavailableDialog}
          style={{ width: "50vw" }}
          onHide={() => setVisibleFeedbackUnavailableDialog(false)}
          footer={footerFeedbackUnavailableContent}
        >
          <p className="m-2" style={{ color: "#fff" }}>
            We're sorry, but feedback is currently unavailable for this
            appointment as its status is{" "}
            <Chip
              className="capitalize"
              color={statusColorMap[selectedAppointment?.status]}
              size="sm"
